      ...
      >
        ...
        >
          <Tile
            ...
            color={rt == null ? undefined : rt < 500 ? "var(--sky-primary)" : rt < 1500 ? "var(--sky-dark)" : "var(--sky-dark)"}
          />
          <Tile
            label="SSL"
          ...
          <Tile
            ...
            color={report.dns_ok === true ? "var(--sky-primary)" : report.dns_ok === false ? "var(--sky-dark)" : undefined}
          />
        </div>
...
function Tile({ label, value, color }: { label: string; value: string; color?: string }) {
  return (
    <div style={{ background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 0, padding: "0.625rem 0.75rem" }}>
      <p style={{ fontSize: "0.65rem", color: "var(--muted)", textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: 600 }}>{label}</p>
      <p style={{ fontSize: "1rem", fontWeight: 800, color: color ?? "var(--text)", marginTop: "0.2rem" }}>{value}</p>
    </div>
  );
}
